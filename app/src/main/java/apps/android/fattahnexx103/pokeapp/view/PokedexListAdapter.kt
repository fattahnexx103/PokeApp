...
class PokedexListAdapter(private val itemList: ArrayList<PokeemonModel> = arrayListOf()):RecyclerView.Adapter<PokedexListAdapter.ItemViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        holder.view.pokdex_item_name.text = itemList[position].forms!![0].name
        ...
        holder.view.item_layout.setOnClickListener {
            Toast.makeText(holder.view.context, "ITEM..... ${position} ... is... ${itemList[position].forms!![0].name}",Toast.LENGTH_LONG).show()
            val action = PokedexFragmentDirections.actionPokedexFragmentToPokemonFragment(itemList[position])
